...
module.exports = {
    ...
    // Generate JWT for refresh token which is save in http cookie
    issueRef(payload, jwtOptions = {}) {
        _.defaults(jwtOptions, strapi.plugins['users-permissions'].config.jwtRefresh);
        return jwt.sign(
            _.clone(payload.toJSON ? payload.toJSON() : payload),
            _.get(strapi.plugins, ['users-permissions', 'config', 'jwtRefreshSec']),
            jwtOptions
        );
    },
